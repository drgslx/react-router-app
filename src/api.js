//This code defines an array of session categories and provides functions to retrieve specific sessions, categories, and a simplified list of categories. It allows you to access and work with session data in your application.

// Define an array of session categories with their associated sessions
...
// Function to get a specific session by its sessionId and catId
export function getSession({ sessionId, catId }) {
  // Find the category by catId and then find the session within that category by sessionId
  return (
    //Simulates and API request
    categories
      .find(({ id }) => id === catId)
      .sessions.find(({ id }) => id === sessionId)
  );
}
// Function to get a specific category by its catId
export function getCategory(catId) {
  // Find the category by catId
  return categories.find(({ id }) => id === catId);
}
// Function to get a simplified list of categories with only name and id
export function getCategories() {
  // Map categories to a simplified format containing only name and id
